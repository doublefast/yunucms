...
layui.use(["layer", "util", "common", "element"], function() {
    ...
    $(function () {
        ...
        nav_left.find('.layui-colla-item').each(function (i, v) {

            var self = $(v);

            var title = self.find('.layui-colla-title');

            self.find('.layui-show').removeClass('layui-show').show();

            self.find('.layui-colla-content').css('border-top', 0);

            title.on('click', function () {

                nav_left.find('.layui-colla-icon').html('');

                nav_left.find('.layui-colla-item').not(self).find('.layui-colla-content').slideUp();

                var self_content = self.find('.layui-colla-content');

                if (self_content.is(':hidden')) {

                    self_content.slideDown();

                    self.find('.layui-colla-icon').html('')

                } else {

                    self_content.slideUp();
